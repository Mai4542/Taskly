...
const TaskCard = ({ task, status, onClick }: TaskCardProps) => {
  ...
  return (
    <div
      ref={ref}
      onClick={onClick}
      className={`cursor-grab active:cursor-grabbing rounded-lg p-4 flex flex-col gap-3 h-27 transition-opacity ${
        isDragging ? 'opacity-40' : 'opacity-100'
      } ${
        isBlockedOrDelayed
          ? 'bg-[#FFDAD633] border border-[#BA1A1A1A]'
          : isToday
            ? 'bg-white border border-transparent border-l-4 border-l-primary'
            : 'border-surface-low bg-white'
      }`}
    >
      <p className="text-neutral-high text-[14px] font-semibold leading-snug">
        {task.title}
      </p>
        ...
        {dueMeta ? (
          dueMeta.variant === 'delayed' ? (
            <div className="flex flex-row items-center mt-4 gap-1 ">
              ...
              <span className="text-[#DC2626] text-[11px] font-bold uppercase">
                Delayed
              </span>
            </div>
            ...
            <div className="flex flex-row items-center mt-4 gap-2">
              <ColoredCalendar
                ...
                className="w-3.5 h-3.5"
              />
              <span className="text-neutral-medium text-[11px] font-semibold uppercase">
                {dueMeta.label}
              </span>
            </div>
          )

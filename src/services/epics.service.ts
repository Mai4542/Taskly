...
export interface SearchEpicsParams extends PaginationParams {
  searchTerm?: string;
}
...
export const searchProjectEpics = async (
  projectId: string,
  { page = 1, limit = 10, searchTerm = '' }: SearchEpicsParams = {},
): Promise<PaginatedResponse<Epic>> => {
  const offset = (page - 1) * limit;

  let url = `${REST_BASE_URL}/project_epics?project_id=eq.${projectId}`;
